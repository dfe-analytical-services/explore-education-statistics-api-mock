...
CREATE TABLE data_normalised(id UINTEGER PRIMARY KEY DEFAULT(nextval('data_seq')), time_period VARCHAR, time_identifier VARCHAR, geographic_level VARCHAR, country_id UINTEGER, region_id UINTEGER, qualification_TR UINTEGER, SECTIONNAME UINTEGER, YAG UINTEGER, subject_name UINTEGER, difference VARCHAR, earnings_median VARCHAR, living_in_region VARCHAR, trained_in_region VARCHAR, number_of_providers VARCHAR, difference_prop VARCHAR);
